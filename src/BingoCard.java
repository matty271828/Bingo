...
public class BingoCard {
    ...
    for (int i = 0; i < numberOfRows; ++i){
      for (int j = 0; j < numberOfColumns; ++j){
        if (i == numberOfRows - 1 && j == numberOfColumns - 1) {
          ...
          sb.append(numbers[i][j] + Defaults.getNumberSeparator());
        }
      }
    }
    return sb.toString();
  ...
  public void setCardNumbers(String[] numbersAsStrings) {
    ...
    for (int i = 0; i < numbersList.length; ++i){
      numbers[j][k] = numbersList[i];

      // Check whether end of row has been reached
      if ((k + 1) / numberOfColumns == 1){
        // Reset column position and move to next row
        ++j;
        k = 0;
      }
      else {
  ...
  public void markNumber(int number) {
     ...
     */
    // Flag variable
    boolean aNumberHasBeenMarked = false;

    // Loop to search for matching number
    for (int i = 0; i < numberOfRows; ++i){
      for (int j = 0; j < numberOfColumns; ++j){
        if (numbers[i][j] == number){
          markedOff[i][j] = true;
          aNumberHasBeenMarked = true;
        }
      }
    }

    if (aNumberHasBeenMarked == true){
  ...
  public boolean isWinner() {
     ...
     */
    // Flag variable
    boolean allNumbersMarked = true;

    // Loop through markedOff[][] and change flag variable if unmarked numbers encountered
    for (int i = 0; i < numberOfRows; ++i){
      for (int j = 0; j < numberOfColumns; ++j){
